...
def shift_by_letter(letter, letter_shift):
    ...
    if letter==" ":
        return " "
    number_shift = int(ord(letter_shift) - 65)
    if ord(letter)+number_shift > 90:
        return chr(ord(letter)+number_shift-26)
    elif ord(letter)+number_shift <= 90:
        return chr(ord(letter)+number_shift)
...
def scytale_decipher(message, shift):
    ...
    output = ""
    for i in range(len(message)):
        output += message[(i //(len(message) // shift)) + (i % (len(message) // shift)) * shift]
    return output
